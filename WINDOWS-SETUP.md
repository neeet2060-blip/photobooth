...
**전제**: 위 1~11번 설정(특히 6번 pm2 자동시작, 7번 카메라 자동실행, 8-1번 최초 로그인)이
행사 전에 미리 끝나 있고, 11번 체크리스트로 최소 한 번 리허설을 마쳤다는 전제다. 그게
안 돼 있으면 아래 절차가 "자동으로 켜진다"는 부분부터 어긋난다 — 먼저 그 항목들부터
확인할 것.

다 되어 있다면 당일은 아래만 하면 된다:

1. **프린터부터 켠다.** 전원 On → USB(또는 네트워크) 연결된 상태인지, 준비완료 표시등이
   들어오는지 확인. PC보다 먼저 켜두는 게 안전하다(부팅 순서 꼬임 방지).
2. **PC를 켠다.** Windows 로그인 화면이 뜨면 로그인(자동 로그인 설정을 안 해뒀다면 비밀번호
   입력 필요 — 이건 Windows 계정 비밀번호지, 포토부스 비밀번호가 아니다).
3. **1~2분 기다린다.** 로그인 직후 pm2(6번)가 서버를 자동으로 띄우고, 그 다음
   `start-camera.vbs`(7번, 시작프로그램 폴더에 등록됨)가 서버 준비를 기다렸다가 카메라
   화면을 전체화면으로 자동으로 연다. 이 사이엔 화면이 비어있거나 다른 프로그램들이 뜨는
   게 정상이다 — 손대지 말고 기다린다.
4. **카메라 화면이 손님용 화면으로 바로 뜨는지 확인.** 로그인 페이지가 대신 뜨면 8-1번의
   "최초 1회 로그인"이 안 돼 있었다는 뜻 — 비밀번호(`secrets/password.txt`) 입력해서
   한 번 로그인해두면 그 다음부터는 안 뜬다.
   자동으로 아무것도 안 뜨면 수동으로 `http://localhost:3001/camera`를 열어본다(7번 참고
   — 이 포트라 인증서 경고 없이 바로 뜬다).
5. **와이파이/인터넷 연결 확인.** 결제 확인(TOK2026 관리자 → 자동 촬영전환)과 카드결제
   자동확정은 인터넷이 있어야 동작한다. 행사장 와이파이가 불안정하면 이 시점에 미리
   파악해두는 게 낫다.
6. **테스트 한 번 실제로 돌린다.** 오픈 전에 결제수단 선택 → (직접) 결제확인 → 촬영 →
   인쇄까지 한 번 실제로 해본다. 4x6 크기로 잘리지 않고 나오는지까지 확인.
7. 문제없으면 오픈. 문제가 있으면 아래 "당일 문제 생기면" 참고.

### 당일 문제 생기면 (빠른 확인 순서)

- **서버 자체가 안 뜬 것 같다** (아무 화면도 안 열림, 오래 기다려도 그대로): 관리자 권한
  PowerShell에서 `npx pm2 list`로 `photobooth`가 `online`인지 확인. `errored`거나 안 보이면
  `cd C:\photobooth && npx pm2 restart ecosystem.config.cjs --update-env`로 재시작(10번 참고
  — 이름만으로 재시작하면 안 될 때가 있다는 게 이번에 확인된 함정).
- **카메라 화면이 자동으로 안 열림**: 시작프로그램 폴더(`Win+R` → `shell:startup`)에
  `start-camera.vbs` 바로가기가 그대로 있는지 확인, 없으면 다시 만들거나 수동으로
  `http://localhost:3001/camera` 접속.
- **인쇄가 안 나옴**: 프린터 전원/케이블 먼저 확인, Windows 설정에서 그 프린터가 오프라인
  상태로 표시되는지 확인. `/admin` 인쇄 대기열 탭(6번 관련)에서 실패한 작업이 있는지 확인.
- **결제확인 후 촬영으로 안 넘어감**: 와이파이 연결 확인이 최우선(5번). 그 다음 pm2가
  최근에 재시작됐는지 확인 — 재시작 시 진행 중이던 결제 세션은 자동 복구되지만
  (`SESSION-PERSISTENCE.md`), 복구 자체가 안 될 정도로 오래됐거나 알 수 없는 오류면
  `npx pm2 logs photobooth --lines 50`으로 최근 로그 확인.
