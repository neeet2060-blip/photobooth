...
test('captureViaDslr forces JPEG image format, then captures, and saves the file at {PHOTOS_DIR}/{sessionId}/{index}.jpg', async () => {
  const calls = [];
  const execFileImpl = async (cmd, args) => {
    calls.push({ cmd, args });
    if (args[0] === '--capture-image-and-download') {
      // Simulate gphoto2 writing the downloaded shot to the temp filename
      // it was told to use — the real binary does this itself; the stub
      // has to do it too so the subsequent rename() has something to
      // rename.
  ...
  // Real-camera finding (2026-08-18): the EOS 550D's mode dial was set to
  // "RAW + L" and --capture-image-and-download silently saved the RAW file
  // under the .jpg name — a CR2 that loads as a file but decodes as
  // nothing downstream. This call must happen, and it must happen first.
  assert.equal(calls[0].cmd, 'gphoto2');
  assert.deepEqual(calls[0].args, ['--set-config', 'imageformat=0']);

  assert.equal(calls[1].cmd, 'gphoto2');
  assert.deepEqual(calls[1].args.slice(0, 2), ['--capture-image-and-download', '--filename']);
  assert.ok(calls[1].args.includes('--force-overwrite'));
  ...
  const tmpArgUsed = calls[1].args[calls[1].args.indexOf('--filename') + 1];
